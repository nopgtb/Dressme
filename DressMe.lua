...
local mainFrameTitle = "|ccff6ff98DressMe |cffffffff1.4.0|r"
...
do
    ...
    for s in ("human,nightelf,dwarf,gnome,draenei,orc,scourge,tauren,troll,bloodelf,deathknight,pandaren,worgen,goblin"):gmatch("%w+") do
...
mainFrame.buttons.send = CreateFrame("Button", "$parentButtonSend", mainFrame, "UIPanelButtonTemplate")
...
mainFrame.buttons.reset = CreateFrame("Button", "$parentButtonReset", mainFrame, "UIPanelButtonTemplate")
...
mainFrame.buttons.undress = CreateFrame("Button", "$parentButtonUndress", mainFrame, "UIPanelButtonTemplate")
...
mainFrame.buttons.useTarget = CreateFrame("Button", "$parentButtonUseTarget", mainFrame, "UIPanelButtonTemplate")
...
local function slot_Reset(self)
    ...
    local slotId = GetInventorySlotInfo((characterSlotName.."Slot"):upper())
...
local function btnReset_Hook()
    mainFrame.dressingRoom:Undress()
    for _, slot in pairs(mainFrame.slots) do
        if slot.slotName == rangedSlot and ("WARRIORROGUEMAGEWARLOCKDRUIDSHAMANPALADINDEATHKNIGHTMONKPRIEST"):find(classFileName) then
            slot:RemoveItem()
        else
            slot:Reset()
        end
...
do
    ...
    local btnSave = CreateFrame("Button", "$parentButtonSave", scrollFrame, "UIPanelButtonTemplate")
    ...
    local btnSaveAs = CreateFrame("Button", "$parentButtonSaveAs", scrollFrame, "UIPanelButtonTemplate")
    ...
    local btnRemove = CreateFrame("Button", "$parentButtonRemove", scrollFrame, "UIPanelButtonTemplate")
    ...
    local btnTryOn = CreateFrame("Button", "$parentButtonTryOn", scrollFrame, "UIPanelButtonTemplate")
...
do
    ...
    local btnTryOn = CreateFrame("Button", "$parentButtonSaveAs", scrollFrame, "UIPanelButtonTemplate")
    btnTryOn:SetSize(90, 20)
    btnTryOn:SetPoint("LEFT", frame, "BOTTOMLEFT", 0, -12)
    btnTryOn:SetText("Try On")
    btnTryOn:SetScript("OnClick", function() PlaySound("gsTitleOptionOK") end)
    ...
    local btnClear = CreateFrame("Button", "$parentButtonClear", scrollFrame, "UIPanelButtonTemplate")
...
local btnDressMe = CreateFrame("Button", "$parent"..addon.."DressMeButton", CharacterModelFrame, "UIPanelButtonTemplate")
...
do  --------- Character background
    ...
    local btnReset = CreateFrame("Button", "$parentResetButton", colorButtonBackground, "UIPanelButtonTemplate")
    ...
    UIDropDownMenu_Initialize(menu, function()
        ...
        local info = UIDropDownMenu_CreateInfo()
        for background in ("color,human,dwarf,nightelf,gnome,draenei,orc,scourge,tauren,troll,bloodelf,deathknight,pandaren,goblin,worgen"):gmatch("%w+") do
